...
#CALL get_list_product(1000001);


#-----------------------------------------------------------------------------
# insert product 
INSERT INTO Product (create_at, update_at) VALUES (NOW(), CURDATE());
...
#CALL UpdateProduct(1000000, 1000000, N'banh trang', 1,N'ngonn', 100, 20, NOW());
...
# choose food category (insert/update)
DROP PROCEDURE IF EXISTS Update_Product_Option;
DELIMITER $$
CREATE PROCEDURE Update_Product_Option(
	 provider_id_ BIGINT,
	 product_id_ BIGINT, 
...
Begin
	...
	ELSE 
		INSERT ProductOption(product_option_id, label, `value`)
		VALUES(product_id_, label_, value_);
--         ON DUPLICATE KEY UPDATE
--         product_option_id = product_option_id_, 
--         label = label_, 
--         `value` = value_;
        
        INSERT ProductOptionDetail(product_id, product_option_id, price)
		VALUES(product_id_, product_option_id_, price_)
        ON DUPLICATE KEY UPDATE
        product_id= product_id_, 
		product_option_id = product_option_id_, 
        price = price_;
...
# CALL Update_Product_Option(1000000, 1000000, 1000001,'Size', 'B', 100)